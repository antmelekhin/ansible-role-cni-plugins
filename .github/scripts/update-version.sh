...
yq eval -i ".argument_specs.main.options.cni_plugins_version.default = \"${LATEST_VERSION}\"" 'meta/argument_specs.yml'

# Repository variables
REPO_NAME=$(git config --get remote.origin.url | sed -e 's|^https://github.com/||')
UPDATE_VERSION_BRANCH="update-to-${LATEST_VERSION}"
UPDATE_VERSION_COMMIT="fix(version): CNI plugins updated to \`${LATEST_VERSION}\` release"
...
# Create Pull Request
PR_MESSAGE="The upstream [CNI plugins](https://github.com/containernetworking/plugins/releases) released new software version - **${LATEST_VERSION}**!\n\nThis automated PR updates code to bring new version into repository."
